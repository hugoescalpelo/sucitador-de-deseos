...
# Función para seleccionar la tarjeta de sonido (card 1)
def select_audio_device_by_card(card_number):
    devices = sd.query_devices()
    for device_index, device_info in enumerate(devices):
        if f"hw:{card_number}" in device_info['name']:
            return device_index
    raise RuntimeError(f"Dispositivo con card {card_number} no encontrado.")

# Seleccionar la tarjeta de sonido 'card 1'
card_number = 1
device_index = select_audio_device_by_card(card_number)

# Asignar el dispositivo de salida de audio
